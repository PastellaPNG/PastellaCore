...
class AdvancedGPUMiner {
  ...
  detectGPUs() {
    try {
      ...
      try {
        ...
        const testKernel = testGPU.createKernel(
          function () {
  ...
  createKawPowKernel(gpuIndex) {
    try {
      ...
      const kawPowKernel = gpu.createKernel(
        function (nonces, cache, headerHash, blockNumber) {
      ...
      console.log(
        chalk.yellow(`⚠️  GPU kernel creation failed for GPU ${gpuIndex}, using CPU fallback: ${error.message}`)
      );
  ...
  async handleCommand(args) {
    ...
    switch (subCmd) {
      ...
      default:
        ...
        console.log(
          chalk.yellow(
            'Available commands: detect, init, start, stop, status, config, benchmark, log, monitor, tune, optimize, recreate, set, cache, debug, selection, performance, fees, earnings'
          )
        );
  ...
  async startAdvancedMining() {
    ...
    const gpuSelection = await new Promise(resolve => {
      rl.question(
        chalk.cyan(`🎮 Select GPU(s) to use (1-${this.availableGPUs}, comma-separated for multiple): `),
        answer => {
          resolve(answer.trim());
        }
      );
    ...
    console.log(
      chalk.white(`🎮 Active GPUs:    ${this.activeGPUKernels.length} (${selectedGPUs.map(i => i + 1).join(', ')})`)
    );
  ...
  showAdvancedStatus() {
    ...
    if (this.isMining && this.startTime) {
      ...
      if (this.currentMiningBlock) {
        ...
        console.log(
          chalk.white(`🎲 Current Nonce:        ${chalk.cyan(this.currentMiningBlock.nonce.toLocaleString())}`)
        );
      }
    }
    ...
    if (activeGPUKernels.length > 0) {
      ...
      console.log(
        chalk.white(`⚡ Total GPU Hash Rate:    ${chalk.green(this.formatHashRate(this.calculateTotalHashRate()))}`)
      );
      if (this.hashRate > 0) {
  ...
  async mineBlocksAdvanced() {
    ...
    try {
      ...
      if (this.currentMiningBlock && this.currentMiningBlock.index === height) {
        ...
        const selectedTransactions = this.selectTransactionsForBlock(pendingTransactions, coinbaseTransaction);
          ...
          pendingTransactions.forEach((tx, index) => {
            if (tx.fee && typeof tx.fee === 'number' && tx.fee > 0) {
              ...
              if (tx.timestamp) {
                ...
                console.log(
                  chalk.white(`     Age: ${chalk.gray(ageMinutes > 0 ? `${ageMinutes}m old` : 'Just added')}`)
                );
              }
            }
          });
  ...
  selectTransactionsForBlock(pendingTransactions, coinbaseTransaction) {
    ...
    const sortedTransactions = [...pendingTransactions].sort((a, b) => (a.timestamp || 0) - (b.timestamp || 0));
    ...
    if (finalCoinbaseAmount > maxReasonableAmount) {
      throw new Error(
        `Coinbase amount ${finalCoinbaseAmount} PAS exceeds reasonable limit (${maxReasonableAmount} PAS)`
      );
    }
    ...
    if (this.showMiningLogs) {
      ...
      console.log(
        chalk.white(
          `📦 Selected ${chalk.green(selectedTransactions.length)}/${chalk.cyan(pendingTransactions.length)} transactions for block`
        )
      );
  ...
  generateCacheForBlock(blockNumber) {
    try {
      ...
      if (this.showMiningLogs) {
        ...
        console.log(chalk.blue('────────────────────────────────────────────────────────────────────────────────'));
        console.log(
          chalk.white(`📦 Generated KawPow cache: ${chalk.green(this.currentCache.length.toLocaleString())} entries`)
        );
        console.log(chalk.white(`⏱️  Generation time: ${chalk.cyan(this.cacheGenerationTime)}ms`));
        console.log(
          chalk.white(`💾 Memory usage: ${chalk.cyan(((this.currentCache.length * 4) / 1024).toFixed(2))} KB`)
        );
        console.log(chalk.blue('────────────────────────────────────────────────────────────────────────────────'));
      }
    } catch (error) {
    ...
    const mineBatch = async () => {
      ...
      try {
        ...
        if (this.showMiningLogs && this.totalHashes % (safeBatchSize * 10) === 0) {
          ...
          console.log(
            chalk.white(
              `   Range: ${chalk.cyan(startNonce.toLocaleString())} → ${chalk.cyan((startNonce + safeBatchSize - 1).toLocaleString())}`
            )
          );
        ...
        try {
          ...
          if (this.showMiningLogs && this.totalHashes % (safeBatchSize * 20) === 0) {
            ...
            if (kernel.isCPUFallback) {
              console.log(
                chalk.white(`🔍 CPU fallback processed ${chalk.cyan(hashResults.length.toLocaleString())} nonces`)
              );
          ...
          try {
            ...
            console.log(chalk.blue('────────────────────────────────────────────────────────────────────────────────'));
            console.log(
              chalk.white(`🔄 Batch size adjusted to ${chalk.cyan(reducedBatchSize.toLocaleString())} for stability`)
            );
        ...
        if (blockFound) {
          ...
          if (this.showMiningLogs) {
            ...
            console.log(
              chalk.white(`⏱️  Mining Time: ${chalk.cyan((Date.now() - this.startTime).toLocaleString())}ms`)
            );
            console.log(chalk.white(`🔧 Algorithm: ${chalk.cyan('KawPow (ProgPoW + Keccak256)')}`));
            console.log(chalk.blue('────────────────────────────────────────────────────────────────────────────────'));
          }
  ...
  async tunePerformance() {
    ...
    for (const config of testConfigs) {
      try {
        ...
        console.log(
          chalk.white(
            `✅ ${config.threads} threads, ${config.batchSize.toLocaleString()} batch: ${hashRate.toFixed(2)} H/s (${duration}ms)`
          )
        );
    ...
    for (const threads of threadConfigs) {
      for (const batchSize of batchConfigs) {
        try {
          ...
          if (hashRate > bestHashRate && duration < 30000) {
  ...
  showGPUSelection() {
    ...
    console.log(
      chalk.white(`⚡ Combined Hash Rate: ${chalk.green(this.formatHashRate(this.calculateTotalHashRate()))}`)
    );
